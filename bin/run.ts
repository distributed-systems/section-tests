...
const patterns = argv.filter((value, index) => {
    ...
    if (value === '--json-summary') return false;
    if (value === '--test-logs') return false;
    if (value.startsWith('--jobs=')) return false;
    if (value === '--jobs') return false;
    if (jobsIndex >= 0 && index === jobsIndex + 1) return false;
    if (value.startsWith('--timeout=')) return false;
    if (value === '--timeout') return false;
    if (timeoutIndex >= 0 && index === timeoutIndex + 1) return false;
    if (value.startsWith('--timeout-grace=')) return false;
    if (value === '--timeout-grace') return false;
    if (timeoutGraceIndex >= 0 && index === timeoutGraceIndex + 1) return false;
    return value[0] !== '-';
});
