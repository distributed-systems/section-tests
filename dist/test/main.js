...
async function createRunnerFixture(shouldFail) {
...
section('Runner output', (section) => {
    section.test('keeps human output unchanged by default', async () => {
        ...
        const result = spawnSync(process.execPath, [join(rootDirectory, 'dist', 'bin', 'run.js'), './success.test.mjs'], {
    ...
    section.test('emits machine-readable suite summary when enabled', async () => {
        ...
        const result = spawnSync(process.execPath, [join(rootDirectory, 'dist', 'bin', 'run.js'), '--json-summary', './failure.test.mjs'], {
            cwd: fixtureDirectory,
            encoding: 'utf8',
            env: process.env,
        });
